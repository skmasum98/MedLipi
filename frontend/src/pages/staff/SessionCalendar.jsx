...
import { Link } from 'react-router'; // Changed 'react-router' to 'react-router-dom'
...
function SessionCalendar() {
    ...
    // Default Date to Today (Local Format YYYY-MM-DD)
    // new Date().toISOString() can sometimes be 'yesterday' if it's early morning locally.
    // Use this safer method for local date default:
    ...
    const handleCreate = async (e) => {
        ...
        // Overlap Check (Simplified: Check same date + shift name)
        const conflict = sessions.some(s => {
             // Extract date string part to compare reliably
             const sDate = typeof s.date === 'string' ? s.date.substring(0, 10) : new Date(s.date).toISOString().substring(0, 10);
             return sDate === newSession.date && s.session_name === newSession.session_name;
        });
        ...
        try {
            ...
            if (res.ok) {
                ...
                setNewSession({...newSession, date: nextDateStr});
            } else {
        ...
        <div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow-lg border border-gray-100 m-8">
            <div className="flex justify-between items-center mb-6 pb-6 border-b">
                <div>
                    ...
                    <div className="flex items-center gap-2 mt-1 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100 w-fit">
                        <span className="text-indigo-400">📅</span>
                        <span className="text-indigo-800 text-sm font-bold">
                            Dr. {doctorNameDisplay || doctor?.full_name || '...'}
                        </span> 
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2">
                    <h3 className="font-bold text-gray-700 mb-4">Active & Upcoming Sessions</h3>
                    ...
                    <div className="space-y-4">
                        {/* Iterate sessions */}
                        {sessions.map(s => {
                             // Status Checks with Timezone safety
                             // Force string parsing instead of Date obj conversion for comparison
                             const sDateStr = String(s.date).substring(0, 10);
                             const todayStr = new Date().toISOString().substring(0, 10);

                             const isExpired = sDateStr < todayStr;
                             const isFull = s.booked_count >= s.max_patients;
                             const percentage = Math.round((s.booked_count / s.max_patients) * 100);
                             
                             // Display Date (Local formatting)
                             const displayDate = new Date(s.date).toLocaleDateString(undefined, {
                            ...
                            return (
                                <div key={s.schedule_id} className={`flex flex-col md:flex-row items-center border rounded-lg p-4 transition hover:shadow-md ${isExpired ? 'bg-gray-50 opacity-60' : 'bg-white'}`}>
                                    ...
                                    {/* Left: Date Block */}
                                    <div className="flex flex-col items-center justify-center p-3 rounded-md bg-gray-100 min-w-20 w-24">
                                        <span className="text-xs font-bold uppercase text-gray-500">{displayDate.split(',')[0]}</span>
                                        <span className="text-2xl font-bold text-gray-800">{new Date(s.date).getDate()}</span>
                                        <span className="text-xs font-bold text-gray-500">{new Date(s.date).toLocaleString('default', { month: 'short' })}</span>
                                    </div>

                                    {/* Center: Details */}
                                    <div className="flex-1 md:ml-6 text-center md:text-left my-2 md:my-0">
                                        <div className="flex items-center justify-center md:justify-start gap-3">
                                            <h4 className="font-bold text-lg text-indigo-900">
                                                {s.session_name}
                                            </h4>
                                            {isExpired ? <span className="bg-gray-200 text-gray-600 text-[10px] px-2 py-0.5 rounded uppercase font-bold">Ended</span>
                                            : isFull ? <span className="bg-red-100 text-red-600 text-[10px] px-2 py-0.5 rounded uppercase font-bold">Full</span> 
                                            : <span className="bg-green-100 text-green-700 text-[10px] px-2 py-0.5 rounded uppercase font-bold">Open</span>}
                                        </div>
                                        
                                        <p className="text-gray-500 text-sm font-mono mt-1 flex items-center gap-1 justify-center md:justify-start">
                                            ⏰ {s.start_time.substring(0,5)} - {s.end_time.substring(0,5)}
                                        </p>
                                        
                                        {/* Progress Line */}
                                        <div className="flex items-center gap-3 mt-3 text-xs w-full max-w-xs mx-auto md:mx-0">
                                            <div className="flex-1 bg-gray-200 rounded-full h-2 overflow-hidden">
                                                <div className={`h-full ${isFull ? 'bg-red-500' : 'bg-green-500'}`} style={{width: `${percentage}%`}}></div>
                                            </div>
                                            <span className="font-bold text-gray-600 whitespace-nowrap">{s.booked_count} / {s.max_patients}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex md:flex-col gap-2 mt-2 md:mt-0">
                                        <button 
                                            ...
                                            className="text-xs border border-indigo-200 text-indigo-600 px-3 py-1.5 rounded hover:bg-indigo-50 font-bold flex items-center gap-1 justify-center"
                                        >
                                            <span>content_copy</span> Copy Next
                                        </button>
                        ...
                        {sessions.length === 0 && <p className="text-center py-10 text-gray-400 border-2 border-dashed rounded-lg bg-gray-50">No sessions found. Create your first one!</p>}
                    </div>
                    }
                </div>
            </div>
