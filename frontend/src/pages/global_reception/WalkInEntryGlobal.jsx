...
// --- 1. LOAD AVAILABLE SESSIONS ---
    useEffect(() => {
        // Logic to determine Doctor ID
        const targetDoctorId = forceDoctorId 
            ? forceDoctorId 
            : (user?.role === 'doctor' ? user.id : (user?.doctorId || user?.parentId));

        const fetchSessions = async () => {
            // Guard Clause: Don't fetch if no target (e.g. before auth loads or no doctor selected)
            if (!targetDoctorId) return;

            try {
                // FIX: Append '?doctor_id=' to the URL
                ...
                if (res.ok) {
                    ...
                    const todayStr = getDhakaDateISO(); // Use your helper

                    // Filtering Logic
                    const available = data.filter(s => {
                        const sDate = s.date.slice(0, 10);
                        // Strict filter: Make sure returned session actually belongs to this doctor 
                        // (Extra safety layer)
    ...
    useEffect(() => {
        ...
        const runSearch = async () => {
            const targetId = forceDoctorId 
                ? forceDoctorId 
                : (user?.role === 'doctor' ? user.id : user?.doctorId);
